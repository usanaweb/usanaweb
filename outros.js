...
function toggleSearch() {
    ...
    var searchButton = document.querySelector("button[aria-label='Pesquisar']");

    // Alterna a visibilidade da div com a barra de pesquisa
    if (searchContainer.classList.contains("d-none")) {
        searchContainer.classList.remove("d-none"); // Torna a div visível
        searchButton.style.display = "none"; // Oculta o botão de pesquisa
    } else {
        searchContainer.classList.add("d-none"); // Torna a div invisível
        searchButton.style.display = "inline-block"; // Torna o botão de pesquisa visível novamente
    }
}
...
function focusContent(event) {
    ...
    // Localiza o elemento com ID 'conteudo-principal'
    const target = document.getElementById('conteudo-principal');
    
    if (target) {
        // Garante que o elemento seja focável (adiciona 'tabindex' se necessário)
